...
def _find_code_txt_path(day_dir: Path, code: str, require_10s: bool = True) -> Path | None:
    ...
    if not candidates:
        return None
    if require_10s:
        ten_sec = [path for path in candidates if path.stem.lower().endswith("_10s")]
        if not ten_sec:
            return None
...
def check_buy_condition_r76_sim(
    ...
    if len(frame) >= 2:
        plbc = _price_lead_breakout_context_sim(
            frame.iloc[-1], frame.iloc[-2], frame, ts, live_price, cross_info, True
        )
        if plbc["can_enter"]:
            candidate_ok = True
            candidate_reason = "PRICE_LEAD|" + plbc["reason"]
    ...
    if not ok:
        ...
        elif SIM_RELAXED_SHARED_GATES:
            relaxed_ok, relaxed_reason = _simulate_relaxed_shared_gate(
                frame=frame,
                live_price=live_price,
                cross_info=cross_info,
                shared_reason=str(reason),
            )
            if relaxed_ok:
                ok = True
    ...
    return True, candidate_reason if candidate_ok else reason
...
def simulate_date(
    ...
    for path in sorted(data_dir.glob("*.txt")):
        ...
        if code is None:
            continue
        stem = path.stem.lower()
        if require_10s_file and not stem.endswith("_10s"):
            continue
        ...
        def _is_preferred(candidate: Path, current: Path) -> bool:
            ...
            c_rank = 0 if c_stem.endswith("_10s") else (1 if c_stem.endswith("_1m") else (3 if c_stem.endswith("_20s") else 2))
            cur_rank = 0 if cur_stem.endswith("_10s") else (1 if cur_stem.endswith("_1m") else (3 if cur_stem.endswith("_20s") else 2))
    ...
    if not csv_files:
        if require_10s_file:
            log(f"ERROR: No _10s TXT files found in {data_dir}")
            log("Hint: run r001 collector (default output) to generate *_10s.txt files.")
        else:
