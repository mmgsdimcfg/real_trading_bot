...
Run examples:
- python xgraph/auto_trading/r76_trade_live_execute.py
- python xgraph/auto_trading/r76_trade_live_execute.py --date 20260508
...
import pandas as pd
from r76_strategy_core_shared import (
    R76StrategyConfig,
...
TODAY_CODE_FILE = current_dir / "r76_trade_watchlist_today.txt"
...
STOP_LOSS_EARLY_PERCENT = -0.020     # 진입 초기(STOP_LOSS_MIN_HOLD_SECONDS ?�내) ?�이�?방�????��? ?�절
STOP_LOSS_MIN_HOLD_SECONDS = 600    # ???�간(�? 미만?�면 EARLY ?�절 기�? ?�용 (기본 10�?
STARTUP_WARMUP_SECONDS = 180        # ?�크립트 ?�작 ?????�간(�? ?�안 ?�규 매수 ?�음 (지???�정??+ 모니?�링 ?�유 보장)
...
BOX_RANGE_HOLD_MAX_RANGE_PCT = 0.0065         # 최근 N�?�?고�???�� ?�재가 ?��?0.65% ?�내
BOX_RANGE_HOLD_MAX_BB_WIDTH_PCT = 0.0080      # 볼린?� 밴드 ??�� ?�재가 ?��?0.80% ?�내
# 매수 2?�계(근접 ARM -> ?�정/강모멘�? 진입) ?�라미터
NEAR_CROSS_ARM_GAP_MAX = 0.0045        # MA5가 BB_MIDDLE ?�래???�더?�도 gap 0.45% ?�내�?ARM ?�보
...
# 기본 ?�칙?� MA5 ?�향?�파 ?�선. ?�외 경로???�정 ?�간창에?�만 ?�한?�으�??�용.
...
# 조기진입(ARM/EARLY) ?�동???�터: ?�량 체결 급등?�로 ?�한 ?�진??방�?
EARLY_NEAR_CROSS_MIN_VOLUME = 800
EARLY_NEAR_CROSS_MIN_VOL_MA = 500
EARLY_NEAR_CROSS_MIN_TURNOVER_KRW = 5_000_000
POLL_INTERVAL_SECONDS = 20
LIVE_PRICE_BB_BUFFER_PCT = 0.0008      # ?�재가가 BB 중간???��?0.08% ?�상 ?�어???�효 ?�파�??�정
LIVE_PRICE_CROSS_CONFIRM_POLLS = 3      # 20�??�링 기�? 3???�속 ?�인 (?�향 ?�로??/ 매수)
LIVE_PRICE_CROSS_CONFIRM_SECONDS = 60   # �?감�? ??최소 60�??��? (?�향 ?�로??/ 매수)
# ?�향 ?�로??매도)??즉시 반응 ???��??�간???�면 급락 구간?�서 매도 기회�??�침
LIVE_PRICE_DOWN_CROSS_CONFIRM_POLLS = 1     # ?�향 ?�로?? 1??감�? 즉시 ?�호 발동
LIVE_PRICE_DOWN_CROSS_CONFIRM_SECONDS = 0   # ?�향 ?�로?? 지???�이 즉시 ?�인
...
OBV_BREAKOUT_LOOKBACK_BARS = 5  # OBV가 최근 N�?�?고점???�파?�는지 ?�인
...
VOLUME_RATIO_STRONG_RELAX = 0.10  # ADX 강추?????�화 ??
...
# domestic_stock_functions / inquire_time_itemchartprice ???�이브러리에??
...
# ????모듈??module-level logging.info/warning ?�로 루트 로거�?직접 ?�용?�는 경우
# 루트 로거 ?�체????��???�들???�벨�??�터링할 ???�으므�? ?�???�래 ?�터�?추�?
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ADX / DI - 추세 강도 (Wilder's smoothing, ?�보???�신???�터)
    tr = pd.concat([
        out["high"] - out["low"],
...
def _buy_support_score(cur: pd.Series, prev: pd.Series, frame: pd.DataFrame | None = None) -> int:
    ...
    # 4) MACD: MACD > Signal + ?�스?�그??가???�수 ?�는 ?�수축소)
...
class TradingAPI:
    ...
    def get_affordable_buy_qty(self, code: str, price: float, now: datetime, nxt_tradeable: bool) -> int:
        ...
        if qty_by_psbl <= 0:
            # 주문가???�답?� ?��?�??�량/금액 ?�석?????�면 과주�?방�?�??�해 보수?�으�?0�?처리
...
def run(target_date: str | None = None) -> None:
    ...
    while True:
        ...
        # 15:20~15:30 ?�규???�시?��? 구간?� 종목�??�그??체크�?건너?�고
        # ?�시?��? �?�� 로직(?�일 매수 + ?�익 구간)�??�행?�다.
