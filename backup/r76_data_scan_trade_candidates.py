...
Run examples:
- python xgraph/auto_trading/r76_data_scan_trade_candidates.py --date 20260508
- python xgraph/auto_trading/r76_data_scan_trade_candidates.py --date 20260508 --max-codes 20
...
# r73 매수 로직 ?�렬???�계�?LIVE_PRICE_BB_BUFFER_PCT = 0.0
...
# ?�로??지???�계�?DI_PLUS_MIN_STRENGTH = 20.0  # DI+ 최소�?(?�향 강도)
DI_DIFFERENCE_MIN = 5.0       # DI+ - DI- 최소�?(추세 ?�실??
MACD_HIST_BUY_SIGNAL = True   # MACD Histogram ?�수 ?��? ?�인
OBV_UPTREND_CHECK = True      # OBV가 OBV_MA보다 ?��? ?�인
...
def filter_data_by_date(df, target_date):
    ...
    if isinstance(df.index, pd.DatetimeIndex):
        # collector ?�짜 결합 ?�류(1900??가 ?�인 경우 ?�당 ?�코?�만 ?�거?�고 목표 ?�짜�??�용?�니??
...
def build_indicators(df):
    ...
    # CSV?�서 직접 로드????��?��? ?�용?�고, ?�는 ??��?�만 계산
    # ATR?� ??�� 계산 (CSV???�음)
    df["ATR"] = calc_atr(df, length=14)

    # MA_5??CSV???��?�? MA_20�??�위?�환??MA5/MA20 ?�성/?�인
    ...
    df["MA20"] = df["MA_20"]  # ?�위?�환

    # Bollinger Bands - CSV???��?�??�으�?계산
    ...
    # Volume & Amount - CSV??VOL_MA20?� ?��?�??�으�?계산
...
def calculate_candidate_score(candidate, config):
    ...
    # r73 보조지???�수(0~8)�?최�? 32?�으�?반영 (?�로??지??추�?�?최�? ?�수 증�?)
    score += min(32.0, support_score * 4.0)

    # r73 live price > BB middle buffer 근접 조건 가??    if live_cross_ready:
        score += 8.0
...
def evaluate_candidate(code, name, df, target_date=None, config=BALANCED_RANKED_CONFIG):
    ...
    # r73 ?�렬 ?�터: ?�심 리스????��?� hard fail, ?�?�밍 민감 ??��?� soft flag�??�화
    ...
    if trend_state == "down":
        ...
        # ?�승 추세�?기본 베이?�로 ?�용: require_trend=True????flat/unknown???�외
...
def build_history_comparison(data_root, stock_list, history_window=DEFAULT_HISTORY_WINDOW):
    ...
    for i, date_dir in enumerate(date_dirs, start=1):
        ...
        print(f"  [{i}/{total_dates}] {date_dir.name} ?�캔 �?..", end="\r", flush=True)
    ...
    print(f"  ?�스?�리 비교 ?�료 ({total_dates}?�치)          ")
...
def get_latest_data_dir(base_path="data"):
    ...
    if not base.exists():
        raise SystemExit("??data ?�더가 ?�습?�다. collector 먼�? ?�행?�세??")

    dirs = [d for d in base.iterdir() if d.is_dir()]
    if not dirs:
        raise SystemExit("???�이???�더가 비어 ?�습?�다.")
...
def load_symbols():
    symbols_path = Path("r76_universe_symbols_master.csv")

    if not symbols_path.exists():
        raise SystemExit("??r76_universe_symbols_master.csv ?�일???�습?�다.")

    df = pd.read_csv(symbols_path)

    if "code" not in df.columns:
        raise SystemExit("??r76_universe_symbols_master.csv??'code' 컬럼 ?�요")

    # code?� name 컬럼???�께 반환 (name???�으�?�?문자??
...
def filter_stock_list_by_existing_txt(stock_list, data_dir, verbose=True):
    ...
    if verbose:
        ...
        print(
            f"[INFO] {data_dir.name} ?�더 기�? ?�캔 종목 ?�터�? "
            f"{len(stock_list)} -> {len(filtered)} (?�외 {missing})"
        )
...
def scan(
    ...
    if verbose:
        print(f"?�� �?{total} 종목 ?�캔 ?�작 (?�더: {data_dir}, config: {config.name})\n")
    ...
    for idx, (code, name) in enumerate(stock_list, start=1):
        ...
        if verbose:
            if candidate["eligible"]:
                print(f"[{idx}/{total}] {code} ???�보(score={candidate['score']:.2f})")
            else:
                reasons = candidate["fail_reasons"] or [candidate["skip_reason"] or "unknown"]
                print(f"[{idx}/{total}] {code} ???�락 ({', '.join(reasons)})")
    ...
    if verbose:
        print("\n========== ?�캔 ?�료 ==========")
        print(f"�?종목: {total}")
        print(f"?�격 ?�: {summary['eligible_pool_count']}")
        print(f"최종 ?�정: {summary['selected_count']}")
        print(f"?�킵: {skipped}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Scanner (collector ?�동 버전)")
    parser.add_argument(
        "--date",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="?�이???�더 (?? data/20260328)"
    )
    parser.add_argument(
        "--history-window",
        type=int,
        default=DEFAULT_HISTORY_WINDOW,
        help="비교 리포?�에 ?�함??최근 거래????
    )
...
if __name__ == "__main__":
    ...
    if target_date:
        ...
        if not stock_list:
            raise SystemExit(f"??{data_dir} ?�더???�캔 가?�한 TXT 종목???�습?�다.")
    ...
    print(f"???�캐??리포?��? ?�?�되?�습?�다: {report_file}")

    if target_date:
        print(f"\n?�� {target_date.strftime('%Y%m%d')} 기�? 추천 종목:", picks)
    else:
        print("\n?�� 최신 ?�이??기�? 추천 종목:", picks)
