...
r76_data_collect_symbols_daily.py as 20-second bars), then normalizes bars to the same
3-minute strategy cadence used in r76_trade_live_execute.py.

Run examples:
- python xgraph/auto_trading/r76_trade_simulate_by_date.py --date 20260508
- python xgraph/auto_trading/r76_trade_simulate_by_date.py --date 20260508 --codes 003490 018880
...
TODAY_CODE_FILE = SCRIPT_DIR / "r76_trade_watchlist_today.txt"
...
MA5_BB_DOWN_CROSS_MIN_PNL = 0.000   # r73�??�일: 본전 ?�상?�면 ?�로?�다??매도 ?�용
...
BB_SQUEEZE_MIN_WIDTH_PCT = 0.008  # BB ???�단-?�단)/?�재가 < 0.8%?�면 ?�렴/?�보 ?�퀴즈 ??매수 ?�외
...
def is_new_entry_allowed(ts: pd.Timestamp, nxt_tradeable: bool) -> bool:
    """NXT 가???��????�라 매수 ?�용 ?�간 결정.
    - ?�규??09:00-15:20): 모든 종목
    - NXT ?�간(08:00-08:50, 15:30-19:59): NXT 가??종목�?
    """
...
def _buy_support_score(cur: pd.Series, prev: pd.Series, frame: pd.DataFrame | None = None) -> int:
    ...
    # ??MACD: MACD > Signal + ?�스?�그??가???�수 ?�는 ?�수 축소)
...
def run_scheduled_liquidations(
    ...
    # ?�규??15:20 ?�시?��? �?�� (r73?� ?�일):
    # ?�일 매수�?�?regular/morning_nxt ?��??�만, 그리�??�익 구간???�만 �?��
    ...
    # NXT 19:59 강제 �?�� (r73?� ?�일): afternoon_nxt ?�일 매수분만 �?��
...
def _load_nxt_flags(data_dir: Path) -> dict[str, bool]:
    """r76_data_collect_symbols_daily.py가 ?�?�한 nxt_flags.json??로드?�니??

    ?�일???�으�?�?dict �?릏려 ?�출?��? NXT_ELIGIBLE_CODES_FALLBACK�?
    ?�백?�도�??�니??
    """
...
def simulate_date(
    ...
    log(f"Entry window    : 09:00-15:20 (?�규) / NXT ?�션 가??)
    log(f"Force exit      : 15:20 / 19:59 (?�일 ?�량�?��)")
    ...
    # nxt_flags.json (data_collector ?�?????�으�??�선 ?�용, ?�으�?fallback
    ...
    for ts in all_times:
        ...
        for code, frame in frames.items():
            ...
            # ?�전 매매?� ?�일??cross signal tracker/confirm logic ?�용
    ...
    for code in traded_codes:
        ...
        raw(f"    {'?�' * 46}")
        raw(f"    종목 ?�익: {code_pnl:+,.0f} KRW  (매수 {len(buys)}??/ 매도 {len(sells)}??")
    ...
    raw(f"{'?�' * 60}")
    raw(f"  초기 ?�본    : {initial_capital:>15,.0f} KRW")
    raw(f"  최종 ?��??? : {final_value:>15,.0f} KRW")
    raw(f"  ?�현 ?�익    : {realized_pnl:>+15,.0f} KRW")
    raw(f"  �??�익      : {total_pnl:>+15,.0f} KRW  ({total_pnl_pct:+.2f}%)")
    raw(f"  ?�여 ?�금    : {sim.cash:>15,.0f} KRW")
    raw(f"{'?�' * 60}")
    raw(f"  매도 ?�수    : {total_sell_count}?? (?�절 {len(wins)}??/ ?�절·기�? {len(losses)}??")
    raw(f"  ?�률         : {win_rate:.1f}%")
    raw(f"{'=' * 60}\n")

    raw(f"{'=' * 60}")
    raw("?�략 비교 결과 (기본?�략 vs 멀?�필??")
    raw(f"{'?�' * 60}")
    raw(
